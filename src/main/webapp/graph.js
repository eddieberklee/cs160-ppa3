  ...
  $(function () {
    ...
    if (doupdate) {
        ...
        $("td.legendLabel").click(function() {
          company = $(this).text(); // .toLowerCase()
          showPopup();
          var info = {
            "Yahoo": "Welcome to Yahoo!, the world's most visited home page. Quickly find what you're searching for, get in touch with friends and stay in-the-know with the latest.",
          ...
          $("#popup-title").text(company);
          $("#popup-content").text(info[company]);
        });
        $("#black-background").click(function() {
            hidePopup();
        });
        $("#idyahoo").click(function() {
...
function find_max_x(arr) {
  // event though blank, necessary kinda
}
function countCompanies() {
  var counter = 0;
  if ($("#idyahoo").attr('checked') != 'checked') {
    counter++;
  }
  if ($("#idgoogle").attr('checked') != 'checked') {
    counter++;
  }
  if ($("#idmicrosoft").attr('checked') != 'checked') {
    counter++;
  }
  return 3-counter;
}
